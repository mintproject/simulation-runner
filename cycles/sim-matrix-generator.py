...
def _generate_matrix(crop_name, output_file, **kwargs):
    column_names = [
        "unique_id",
        "country",
        "crop",
    ...
    f = output_file if output_file else "sim-matrix-" + crop_name.lower() + ".csv"
    with open(f, "w") as f:
        w = csv.writer(f)
        w.writerow(column_names)
        for row in rows:
            l = ["cycles-" + crop_name.lower() + "-" + str(random.randint(1000000000, 9999999999))]
            for e in row:
...
def _main():
    parser = argparse.ArgumentParser(
        description="Cycles Simulation Matrix Generator."
    )
    parser.add_argument(
        "-o",
        "--output",
        dest="output_file",
        default=None,
        help="Simulation matrix output filename"
    )
    parser.add_argument("crop_name", help="Crop name")
    args = parser.parse_args()

    # validate crop name
    crops = ["Maize", "Sorghum", "Sesame", "Peanut", "Cassava"]
    if args.crop_name not in crops:
        log.error("Error: '%s' is not a valid crop name.", args.crop_name)
        exit(1)

    _generate_matrix(**vars(args))
